...
unsigned int _Esplora::readChannel(byte channel) {
  ...
  /* No need to read the Joystick Button --- OPTIMISATION 
...
boolean _Esplora::joyLowHalf(byte joyCh) {
  return (readChannel(joyCh) < 412) // - JOYSTICK_DEAD_ZONE) 
    ? LOW : HIGH;
}

boolean _Esplora::joyHighHalf(byte joyCh) {
  return (readChannel(joyCh) > 612) // + JOYSTICK_DEAD_ZONE)
    ? LOW : HIGH;
}
...
if (ch == JOYSTICK_RIGHT) return joyLowHalf(CH_JOYSTICK_X);
if (ch == JOYSTICK_LEFT)  return joyHighHalf(CH_JOYSTICK_X);
if (ch == JOYSTICK_UP)    return joyLowHalf(CH_JOYSTICK_Y);
if (ch == JOYSTICK_DOWN)  return joyHighHalf(CH_JOYSTICK_Y);
